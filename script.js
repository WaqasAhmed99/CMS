  ...
  committees.forEach((committee) => {
    ...
    if (memberIndexes.length > 0) {
      ...
      const monthsList = monthDetails
        .map((month) => `${month.monthNumber}. Ap de rahe hain ${month.contribution} Rupee Monthly apko milenge ${month.monthlyAmount} Rupee ${month.date} me (Name: ${month.name})`)
        .join("<br>");

      // Set user name from the first committee
      if (isFirstCommittee) {
        userName = monthDetails[0].name; // Use the name from the first month details
        userDetails.push(`<p>Hello <strong>${userName}</strong>, Umeed krta hun Ap kheriat se honge.</p>`);
